...
function PurchasesTable() {
  ...
  if (customer) {
    ...
    customersId.forEach((x)=>{ 
      customerToShow={...customerToShow, firstName:customers.filter((customers) => customers.id === x).map(x=>x.firstName) , 
                                          lastName:customers.filter((customers) => customers.id === x).map(x=>x.lastName),
                                          City:customers.filter((customers) => customers.id === x).map(x=>x.City) }
    })
