...
function PurchasesTable() {
  ...
  if (customer) {
    ...
    productsToShow = purchases.filter(
      (purchases) => purchases.CustomerID === customer
    );
    dateToShow = purchases.filter(
      (purchases) => purchases.CustomerID === customer
    );
    if (product) {
      productsToShow = purchases.filter(
        (purchases) =>
          purchases.CustomerID === customer && purchases.ProductID === product
      );
    ...
    if (date) {
      dateToShow = purchases.filter(
        (purchases) =>
          purchases.CustomerID === customer && purchases.Date === date
      );
      productsToShow = purchases.filter(
        (purchases) =>
          purchases.CustomerID === customer && purchases.Date === date
    ...
    productsToShow = purchases.filter(
      (purchases) => purchases.ProductID === product
    );
    let Id = productsToShow.map((x) => x.CustomerID);
    let customersId = [...new Set(Id)];

    //customersId.forEach((x)=>{ customerToShow.push(customers.filter((customers) => customers.id === x))})

    //customerToShow = customers.filter((customers) => customers.id === Id.forEach(element => {return element}))
    ...
    if (date) {
      dateToShow = purchases.filter(
        (purchases) =>
          purchases.ProductID === product && purchases.Date === date
      );
    }
  } else if (date) {
            ...
            return (
              <tr key={user.id}>
                ...
                <td>
                  {productsToShow
                    .filter(
                      (productsToShow) => productsToShow.CustomerID === user.id
                    )
                    .map((item) => {
                      return (
                        <div key={item.id}>
                ...
                <td>
                  {dateToShow
                    .filter((dateToShow) => dateToShow.CustomerID === user.id)
                    .map((x) => {
                      return (
                        <div key={x.id}>
                          <div>{x.Date}</div>
